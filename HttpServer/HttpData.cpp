...
const uint32_t DEFAULT_EVENT = EPOLLIN | EPOLLET | EPOLLONESHOT; // TODO  这个是啥EPOLLONESHOT
      ...
      keepAlive_(false) 
{
  ...
  channel_->setConnectCallBack(bind(&HttpData::handleConn, this));
}
...
void HttpData::newEvent() {
  ...
  loop_->epollAdd(channel_, DEFAULT_EXPIRED_TIME);
}
...
ssize_t readn(int fd, std::string &inBuffer, bool &zero) {
  ...
  while (true) {
    ...
    if ((nread = read(fd, buff, MAX_BUFF)) < 0) {
      if (errno == EINTR) // TODO 重点看一下
        continue;
      else if (errno == EAGAIN) { // TODO重点了解一下什么时候回返回这个错误码
        return readSum;
      } else {
        perror("read error");
        return -1;
...
void HttpData::handleRead() {
  ...
  do { // TODO 为什么要用do while，只执行一次，除非是保证无错误用宏
    bool zero = false; 
    int read_num = readn(fd_, inBuffer_, zero); // 
    // LOG << "Request: " << inBuffer_;
    if (connectionState_ == H_DISCONNECTING) { // 假如正在连接，跳出
      inBuffer_.clear();
      break;
    }
    // cout << inBuffer_ << endl;
...
void HttpData::handleWrite() {
  if (!error_ && connectionState_ != H_DISCONNECTED) {
    uint32_t &events_ = channel_->getEvents();
    if (writen(fd_, outBuffer_) < 0) { // 比如回写 正在连接也需要回写吗
      perror("writen");
      events_ = 0;
      error_ = true;
    }
...
void HttpData::handleConn() {
  seperateTimer(); // TODO 时间初始化？名字起得不对
  uint32_t &events_ = channel_->getEvents();
  if (!error_ && connectionState_ == H_CONNECTED) {
    if (events_ != 0) { // 假如没有监听事件？ 0改成无效值
    ...
    // 这一等，便是keepalive_timeout时间。如果守护进程在这个等待的时间里，一直没有收到浏览器发过来http请求，则关闭这个http连接。
      ...
      events_ |= EPOLLET;
      loop_->epollMod(channel_, timeout);
...
void HttpData::handleClose() {
  ...
  std::shared_ptr<HttpData> guard(shared_from_this()); // TODO 这个是为啥
  loop_->epollDel(channel_);
}
...
AnalysisState HttpData::analysisRequest() {
  if (method_ == METHOD_POST) {
    ...
    void *mmapRet = mmap(NULL, sbuf.st_size, PROT_READ, MAP_PRIVATE, src_fd, 0); // TODO 有名管道？
    close(src_fd);
    if (mmapRet == (void *)-1) {
